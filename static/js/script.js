	...
	function onFormSubmit(event){
		...
		// send data to server
			...
			if (response.status = 'OK') {
				var thesisDetail = '<strong>YEAR:</strong> ' + response.data.Year + '<br><strong>TITLE: </strong>' + response.data.Title;
	...
	function loadThesis(){
		...
		$.get(thesis_list_api, {} , function(response) {
			...
			response.data.forEach(function(thesis){
				var thesisDetail = '<strong>YEAR: </strong>' + thesis.Year + '<br><strong>TITLE: </strong>'  + thesis.Title;
